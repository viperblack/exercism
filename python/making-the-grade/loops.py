...
def above_threshold(student_scores, threshold):
    """Determine how many of the provided student scores
    were 'the best' based on the provided threshold.
...
def letter_grades(highest):
    ...
    :return: list - of lower threshold scores for each D-A letter grade interval.
            ...
            86 <= "A" <= 100
    """
    return [41 + int((highest-40)/4 * index) for index in range(4)]
...
def student_ranking(student_scores, student_names):
    ...
    """
    rank = [item + 1 for item in range(len(student_scores))]
    return [f"{rank}. {name}: {str(score)}"
